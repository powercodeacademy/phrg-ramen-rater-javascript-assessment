...
- See all ramen images in the `div` with the id of `ramen-menu`. When the page
  loads, load the ramens and display the image for each of the ramen using an
  `img` tag inside the `#ramen-menu` div.
- Click on an image from the `#ramen-menu` div and fire a callback to see all the info about that
